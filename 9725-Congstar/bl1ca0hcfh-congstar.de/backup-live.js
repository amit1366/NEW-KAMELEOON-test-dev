...
const globalGoals = () => {
  ...
  if (/\/checkout\/bestaetigung\/?(\?|#|$)/.test(window.location.href)) {
    ...
    let currentId; // eslint-disable-next-line no-inner-declarations
    ...
    if (/\/checkout\/(warenkorb|bestaetigung)/.test(document.location.href)) {
      Kameleoon.API.Core.runWhenConditionTrue(() => window.dataLayer && getCheckoutEvent() && getBasketItemsEvent(), () => {
        ...
        if (/\/checkout\/warenkorb/.test(document.location.href)) {
          ...
          Kameleoon.API.Goals.processConversion(goals['[GG] Order and Revenue'], cartValues.summ); // console.log('[AI]OrderAge', getCustomerAge());

          if (getCustomerAge()) Kameleoon.API.Data.setCustomData('[AI]OrderAge', getCustomerAge()); // console.log('[AI]OrderOption', cartValues.option);
...
function pushScoreSegment(cdname) {
  ...
  if (data.groupTwo !== cdname) {
    ...
    localData.set('Group Segment Behaviour', { ...data,
      groupTwo: cdname
    });
  }
}
...
class Bestandskunde {
  ...
  criteriaFour() {
    const regexCongstarApp = RegExp(/utm_source=app-congstar/);
    const regexMediumApp = RegExp(/utm_medium=app/);
...
class Interessent {
  ...
  conditionOne() {
    const url = window.location.href;
    const googleAdWordsCheckOne = new RegExp(/^.+\.google(\.com?)?\.[^/\.]+\/aclk([\?&#].*|)$/);
    const googleAdWordsCheckTwo = new RegExp(/[\?&#]gclid=/);
    const googleContentNetzwerk = new RegExp(/(\?gdnid=\d*)(&glid=\w*)?(&lpid=\d*)?(&dclid=.*)?/);
...
class Schnappchenjager {
  ...
  conditionFive() {
    ...
    const producerPhone = new RegExp(/handys\/\w+\/$/);
...
class Unentschieden {
  ...
  conditionTwo() {
    const tarifPrepaid = new RegExp(/prepaid\/\w+/);
    const tarifHandy = new RegExp(/handytarife\/\w+/);
...
class UnentschiedenSmartphone {
  ...
  conditionOne() {
    ...
    const timerId = setInterval(() => {
      ...
      localData.set(this.goalCdName, { ...data,
