...
const pdpBehaviour = {
    ...
    getPrice: () => {
        const [totalPriceInfo] = window.dataLayer.filter(
            (data) => data.ecommerce && data.ecommerce.detail && data.ecommerce.detail.products && data.ecommerce.detail.products.length,
        );
    ...
    conditionFindTotalPrice: () => (
        window.dataLayer
            && window.dataLayer.filter((data) => data.ecommerce && data.ecommerce.detail && data.ecommerce.detail.products && data.ecommerce.detail.products.length).length
    ),
    findUrl: () => {
...
class Interessent {
    ...
    conditionThree() {
        ...
        if (window.location.href === visitPage) {
...
class Premium {
    ...
    conditionThree() {
        const chooseTarif = new RegExp(/(18gb)/gi);
...
class Schnappchenjager {
    ...
    conditionTwo() {
        if (this.url === this.prepraidSmallTarifUrl || this.url === this.tarifSmallTraifUrl) {
            const sumScore = 2;
    ...
    fiveConditionBehaviour() {
        ...
        Kameleoon.API.Core.runWhenElementPresent('input[name="price"]', (inputs) => {
            const radioGoalsBehaviour = {
                ...
                'radio-price-monthlyPrice': function (num, id) {
                    if (num < 20) {
                        Kameleoon.API.Goals.processConversion(goals['[GG] Filter monatlicher Preis <20€']);
                        this.addScore(id);
                    }
                },
                'radio-price-oneTimePrice': function (num, id) {
                    if (num < 500) {
    ...
    conditionSix() {
        const chooseTarif = new RegExp(/(5gb)/gi);
...
class Unentschieden {
    ...
    conditionOne() {
        const isTarifPage = new RegExp(/(handytarife|prepaid)\/\w+/);
    ...
    conditionThree() {
        const activeUrl = 'https://www.congstar.de/handytarife/smartphone-tarife-im-vergleich/';
...
const T02CscMessagebordDealcloser = () => {
    const goals = {
        '[GG] Klick auf in den Warenkorb': 238183
    };
        ...
        Kameleoon.API.Core.runWhenElementPresent(
            ...
            ([elements]) => {
                elements.addEventListener('click', () => {
                    Kameleoon.API.Goals.processConversion(goals['[GG] Klick auf in den Warenkorb']);
                });
            }
        );
    }
...
const T16ProduktanordnungPremiumsegment = () => {
    const goals = {
...
const T21EnteaTarifseiten = () => {
    const goals = {
        'T21 A2C LTE50': 273755,
        'T21 A2C Extra Daten': 273754,
        '[GG] Order Extra Daten ': 273758,
        'T21 A2C Disney Option': 273753,
        '[GG] Order Disney Option': 273759,
        'T21 A2C Musik Option': 273752,
        'T21 24 Monate chosen': 273756,
        'T21 Ohne Laufzeit chosen': 273757,
    };
    ...
    if (window.location.href.match(/\/checkout\/warenkorb\/?(\?|#|$)/)) {
        Kameleoon.API.Core.runWhenConditionTrue(
            ...
            () => {
                ...
                basketItems.forEach((product) => {
                    if (
                        product.name.match(/AllnetFlat[SML]/)
                        || product.name.match(/AllnetFlat[SML]Flex/)
                    ) {
                        let plan = null;
                        if (sessionStorage.getItem(sessionStorageNames.plan24) === 'true') {
                            ...
                            Kameleoon.API.Goals.processConversion(goals['T21 Ohne Laufzeit chosen']);
                            plan = 'Ohne Laufzeit';
                        }
                        if (plan) {
                ...
                Object.values(sessionStorageNames).forEach((value) => {
                    if (value !== sessionStorageNames.plan24) {
...
const getProduct = (storageName) => sessionStorage.getItem(storageName) 
...
const T20HomespotDatenLGeofancing = () => {
    const goals = {
...
T14Entea();
T17Dealcloser();
T02CscMessagebordDealcloser();
T16ProduktanordnungPremiumsegment();
T03CscTarifwechsel();
T04CscTarifwechsel();
T21EnteaTarifseiten();
T20HomespotDatenLGeofancing();
T22PrepaidTidal();
T23SchnappchenjagerCase2();
T24PrepaidNaviWording();
