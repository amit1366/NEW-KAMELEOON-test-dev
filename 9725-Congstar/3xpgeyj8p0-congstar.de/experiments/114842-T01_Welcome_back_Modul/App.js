    ...
    async generateTiles() {
        ...
        try {
            ...
            for (let action of actions) {
                ...
                if (this.countCard > 3) {
                    this.data[action] = this.data[action].filter((tile, index) => index < 5) //count tiles
                } else {
                    this.data[action] = this.data[action].filter((tile, index) => index < 2) //count tiles
                }
            ...
            } else if (idsForRequest.device && idsForRequest.device.length > 1 && !idsForRequest.plan) {
                url = `https://customers.kameleoon.com/congstar/product-api/product-data?product=${idsForRequest.device.join(',')}`;
            } else if (!idsForRequest.device && idsForRequest.plan && idsForRequest.plan.length > 1) {
            ...
            if (data) {
                if (data['plan']) {
                    const ids = Object.keys(data['plan']);
                    const plans = [];
                    for (let id of ids) {
                        ...
                        plans.push({ ...data['plan'][id], newUsps, media, footnotes });
                    }
                    data['plan'] = plans;
                }
                ...
                if (data.plan && data.device) {
                    if (data.device.length > 4) {
                        data.device = data.device.filter((item, index) => index < 4 && item)
                    }
                }
    ...
    view(data) {
        ...
        if (data['plan']) {
            if (data['device'] && data['device'].length) {
                ...
                data['plan'].forEach((tile, index) => {
                    const item = { ...tile, type: 'plan' }
                    if (index < 2) {
                        tilesReady.push(tileLayouts['plan'](item));
                    }
                })
            }
        }
        ...
        if (this.countCard === 2 && tilesReady.length > 1) {
            tilesReady = tilesReady.slice(0, 2);
        }
        ...
        return (`
            <section id="kameleoonElement" class="kam-come-back ${tilesReady.length < 3 ? Object.keys(data).length < 2 ? 'bg-full' : 'bg-hulf' : ''}">
                ...
                <img class="kam-come-back__image" src="https://storage.kameleoon.eu/congstar/welcome_back_modul/welcome-image-2020-year.png"/>
                <div class="kam-come-back__teasers">
                    ${tilesReady.join('')}
                </div>
            </section> 
        `)
