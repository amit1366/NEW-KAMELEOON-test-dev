...
const goals = {
    ...
    '[DEV|T03|CSC] - Klick CTA - Zum Tarif - 2nd tile': 241818,
...
const tarif2 = String(Kameleoon.API.CurrentVisit.customData['[DEV] CSC T03 Tarif 2']); // 394
...
function addGoalListener() {
    ...
    Kameleoon.API.Core.runWhenElementPresent(goalSelectors[1], ([elements]) => {
        ...
        elements.addEventListener('click', (evt) => {
            Kameleoon.API.Goals.processConversion(goals['[DEV|T03|CSC] - Click CTA any tile']);
            Kameleoon.API.Goals.processConversion(goals['[DEV|T03|CSC] - Klick CTA - Zum Tarif - 2nd tile']);
            Kameleoon.API.Data.setCustomData('[DEV] CSC T03 Tarifgroesse', evt.target.closest('a').getAttribute('title'));
        });
    });
}
