...
const addModalClickListener = sel => {
  Kameleoon.API.Core.runWhenElementPresent(`#${sel}`, ([btn]) => {
    Kameleoon.API.Utils.addUniversalClickListener(btn, () => {
      ...
      if (/planid=(423|424)/.test(window.location.href) || /handytarife\/allnet-flat-tarife\/allnet-flat-s/.test(window.location.href)) {
        Kameleoon.API.Goals.processConversion(goals['[T25] Klick on Link ANF S']);
      } else if (/planid=(425|426)/.test(window.location.href) || /handytarife\/allnet-flat-tarife\/allnet-flat-m/.test(window.location.href)) {
        Kameleoon.API.Goals.processConversion(goals['[T25] Klick on Link ANF M']);
      } else if (/planid=(427|428)/.test(window.location.href) || /handytarife\/allnet-flat-tarife\/allnet-flat-l/.test(window.location.href)) {
        Kameleoon.API.Goals.processConversion(goals['[T25] Klick on Link ANF L']);
      } else if (/planid=(429|430)/.test(window.location.href) || /handytarife\/allnet-flat-tarife\/allnet-flat-xl/.test(window.location.href)) {
  ...
  if (customer === 'new') {
    ...
    if (sendRequest) {
      if (/planid=(423|424|438|439)/.test(window.location.href)) {
        performRequestSelectId('MWVSeFVvclNER1lrQllNSXc0WEkrQT09'); // Tarifwechsel +1 GB (Tarif-IDs 423, 424 = ANF S)
      } else if (/planid=(425|426|427|428|429|430)/.test(window.location.href)) {
        performRequestSelectId('MTdVNXN5MVVNTENTZDVUNXRHdWVwZz09'); // Tarifwechsel +5 GB (Tarif-IDs 425, 426, 427, 428, 429, 430 = ANF M, ANF L & ANF XL)
      }
    }
...
const init = sendRequest => {
  ...
  } else if (/meincongstar\/tarifwechsel\/tarifkonfigurator|meincongstar\/vertragsverlaengerung\/tarifkonfigurator/.test(window.location.href)) {
